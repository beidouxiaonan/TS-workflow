...
class WithdrawDraftTest {
    ...
    @Test void deletesReturnedTicket(){ticket.setStatus("已退回");service.deleteDraft("test","USER-001");verify(repository).deleteDraft("test");}
    @Test void returnedTicketStillRequiresApplicant(){ticket.setStatus("已退回");assertThrows(SecurityException.class,()->service.deleteDraft("test","OTHER"));verify(repository,never()).deleteDraft(anyString());}
    @Test void allOtherStatusesCannotBeDeleted(){for(String status:new String[]{"审批中","处理中","待验收","已完成","未知",null}){ticket.setStatus(status);assertThrows(IllegalStateException.class,()->service.deleteDraft("test","USER-001"));}verify(repository,never()).deleteDraft(anyString());}
    @Test void incorrectNodeCannotBeDeleted(){for(String status:new String[]{"草稿","已退回"}){ticket.setStatus(status);ticket.setCurrentNode("知识上传");assertThrows(IllegalStateException.class,()->service.deleteDraft("test","USER-001"));}verify(repository,never()).deleteDraft(anyString());}
